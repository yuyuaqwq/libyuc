...
#define LIBYUC_ALGORITHM_ARRAY_DEFINE(array_name, id_type, count_type, element_type, key_type, accessor, comparer, referencer) \
    id_type array_name##ArrayFind(element_type* array, count_type count, const key_type* key) { \
    ...
    void array_name##ArrayInsert(element_type* array, count_type count, id_type index, element_type* element) { \
        for(id_type i = count; i > index; i--) { \
            array[i] = array[i - 1]; \
        } \
        array[index] = *element; \
    } \
    void array_name##ArrayDelete(element_type* array, count_type count, id_type index) { \
    ...
    */ \
    ...
    void array_name##ArrayOrderPut(element_type* array, count_type count, const element_type* element) { \
        id_type index = array_name##ArrayOrderFind_Range(array, 0, count - 1, (const key_type*)accessor##_GetKey(array, element)); \
    } \
    bool array_name##ArrayOrderDelete(element_type* array, count_type count, const key_type* key) { \
