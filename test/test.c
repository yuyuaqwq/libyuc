...
int16_t IntBPlusEntryRbBsTreeIteratorPrev(IntBPlusEntryRbBsTree* tree, IntBPlusEntryRbBsStackVector* stack, int16_t cur_id) {
	...
	while (parent_id != ((void*)0)) {
		parent = IntBPlusEntryRbReferencer_Reference(tree, *parent_id);
		if (cur_id != IntBPlusEntryRbAccessor_GetLeft(tree, parent)) break;
		IntBPlusEntryRbReferencer_Dereference(tree, cur);
		cur = parent;
		cur_id = *parent_id;
		parent_id = IntBPlusEntryRbBsStackVectorPopTail(stack);
...
_Bool IntBPlusEntryRbTreeDeleteByIterator(IntBPlusEntryRbTree* tree, IntBPlusEntryRbTreeIterator* iterator) {
	...
	_Bool success = IntBPlusEntryRbTreeDeleteInternal(tree, &iterator->stack, iterator->cur_id);
	iterator->cur_id = IntBPlusEntryRbReferencer_InvalidId;
	return success;
}
...
int16_t IntBPlusEntryRbTreeIteratorLocate(IntBPlusEntryRbTree* tree, IntBPlusEntryRbTreeIterator* iterator, int64_t* key, int8_t* cmp_status) {
	...
	iterator->cur_id = IntBPlusEntryRbBsTreeIteratorLocate((IntBPlusEntryRbBsTree*)tree, &iterator->stack, key, cmp_status);
...
int16_t IntBPlusEntryRbTreeIteratorFirst(IntBPlusEntryRbTree* tree, IntBPlusEntryRbTreeIterator* iterator) {
	IntBPlusEntryRbBsStackVectorInit(&iterator->stack);
	iterator->cur_id = IntBPlusEntryRbBsTreeIteratorFirst((IntBPlusEntryRbBsTree*)tree, &iterator->stack);
	return iterator->cur_id;
}
int16_t IntBPlusEntryRbTreeIteratorLast(IntBPlusEntryRbTree* tree, IntBPlusEntryRbTreeIterator* iterator) {
	IntBPlusEntryRbBsStackVectorInit(&iterator->stack);
	iterator->cur_id = IntBPlusEntryRbBsTreeIteratorLast((IntBPlusEntryRbBsTree*)tree, &iterator->stack);
	return iterator->cur_id;
}
int16_t IntBPlusEntryRbTreeIteratorNext(IntBPlusEntryRbTree* tree, IntBPlusEntryRbTreeIterator* iterator) {
	...
	iterator->cur_id = IntBPlusEntryRbBsTreeIteratorNext((IntBPlusEntryRbBsTree*)tree, &iterator->stack, iterator->cur_id);
	return iterator->cur_id;
}
int16_t IntBPlusEntryRbTreeIteratorPrev(IntBPlusEntryRbTree* tree, IntBPlusEntryRbTreeIterator* iterator) {
	...
	iterator->cur_id = IntBPlusEntryRbBsTreeIteratorPrev((IntBPlusEntryRbBsTree*)tree, &iterator->stack, iterator->cur_id);
	return iterator->cur_id;
}
void IntBPlusEntryRbTreeIteratorCopy(IntBPlusEntryRbTreeIterator* dst_iterator, IntBPlusEntryRbTreeIterator* src_iterator) {
	...
	int16_t count = IntBPlusEntryRbBsStackVectorGetCount(&src_iterator->stack);
	IntBPlusEntryRbBsStackVectorSetCount(&dst_iterator->stack, count);
	for (int16_t i = 0; i < count; i++) {
		*IntBPlusEntryRbBsStackVectorIndex(&dst_iterator->stack, i) = *IntBPlusEntryRbBsStackVectorIndex(&src_iterator->stack, i);
	}
}
...
static void IntBPlusElementSet(IntBPlusEntry* dst_entry, int16_t element_id, IntBPlusEntry* src_entry, IntBPlusElement* element, struct _IntBPlusEntry* element_child_id) {
	...
	if (dst_entry->type == kBPlusEntryLeaf) {
		(dst_element->key = *&element->leaf.key);
		;
	}
	else {
		if (src_entry->type == kBPlusEntryLeaf) {
			...
			(dst_element->key = *&element->index.key);
			if (element_child_id != (((void*)0))) {
				dst_element->index.child_id = element_child_id;
			}
			else {
...
static int16_t IntBPlusElementCreate(IntBPlusEntry* entry) {
	int16_t element_id = (((Int_BPlusEntryExtend*)((uintptr_t)entry - sizeof(Int_BPlusEntryExtend)))->count++, IntBPlusStaticListPop(&((Int_BPlusEntryExtend*)((uintptr_t)entry - sizeof(Int_BPlusEntryExtend)))->static_list, 0)); {
		if (!(element_id >= 0)) {
			{
				printf("");
				__debugbreak();
...
static IntBPlusElement* IntBPlusElementRelease(IntBPlusEntry* entry, int16_t element_id) {
	...
	;
	(((Int_BPlusEntryExtend*)((uintptr_t)entry - sizeof(Int_BPlusEntryExtend)))->count--, IntBPlusStaticListPush(&((Int_BPlusEntryExtend*)((uintptr_t)entry - sizeof(Int_BPlusEntryExtend)))->static_list, 0, element_id));
	return element;
}
...
static void IntBPlusEntryDeleteElement(IntBPlusEntry* entry, IntBPlusEntryRbTreeIterator* element_iterator) {
	...
	;
	int16_t del_element_id = element_iterator->cur_id;
	IntBPlusEntryRbTreeDeleteByIterator(&entry->rb_tree, element_iterator);
	entry->element_count--;
	IntBPlusElementRelease(entry, del_element_id);
}
void IntBPlusEntryInit(IntBPlusTree* tree, IntBPlusEntry* entry, BPlusEntryType type) {
	...
	(((Int_BPlusEntryExtend*)((uintptr_t)entry - sizeof(Int_BPlusEntryExtend)))->count = 0, IntBPlusStaticListInit(&((Int_BPlusEntryExtend*)((uintptr_t)entry - sizeof(Int_BPlusEntryExtend)))->static_list, 5));
}
struct _IntBPlusEntry* IntBPlusEntryCreate(IntBPlusTree* tree, BPlusEntryType type) {
	...
	if (type == kBPlusEntryIndex) {
		entry_id = TEST_BPLUS_ENTRY_ALLOCATOR_CreateBySize(tree, sizeof(IntBPlusEntry) - sizeof(IntBPlusLeafEntry) + sizeof(IntBPlusIndexEntry));
	}
	else {
		entry_id = TEST_BPLUS_ENTRY_ALLOCATOR_CreateBySize(tree, sizeof(IntBPlusEntry) - sizeof(IntBPlusIndexEntry) + sizeof(IntBPlusLeafEntry));
	}
...
void IntBPlusEntryRelease(IntBPlusTree* tree, struct _IntBPlusEntry* entry_id) {
	TEST_BPLUS_ENTRY_ALLOCATOR_Release(tree, entry_id);
}
...
static int16_t IntBPlusEntrySplit(IntBPlusTree* tree, IntBPlusEntry* left, struct _IntBPlusEntry* left_id, IntBPlusEntry* parent, int16_t parent_element_id, IntBPlusEntry** src_entry, IntBPlusElement* insert_element, int16_t insert_id, struct _IntBPlusEntry* insert_element_child_id, struct _IntBPlusEntry** out_right_id, IntBPlusEntryRbTreeIterator* up_element_iterator) {
	...
	int16_t fill_rate = ((((Int_BPlusEntryExtend*)((uintptr_t)left - sizeof(Int_BPlusEntryExtend)))->count) + 1) / 2;
	int16_t right_count = 1, left_count = 0;
	int16_t left_fill_rate = (((Int_BPlusEntryExtend*)((uintptr_t)left - sizeof(Int_BPlusEntryExtend)))->count);
	while (1) {
...
static _Bool IntBPlusTreeInsertElement(IntBPlusTree* tree, IntBPlusIterator* iterator, IntBPlusEntry* src_entry, IntBPlusElement* insert_element, IntBPlusEntryRbTreeIterator* insert_element_iterator, struct _IntBPlusEntry* insert_element_child_id) {
	...
	IntBPlusEntry* old_src_entry = src_entry;
	...
	do {
		int16_t free_rate = (5 - ((Int_BPlusEntryExtend*)((uintptr_t)cur - sizeof(Int_BPlusEntryExtend)))->count);
		int16_t need_rate = 1;
		...
		else if ((5 - ((Int_BPlusEntryExtend*)((uintptr_t)cur - sizeof(Int_BPlusEntryExtend)))->count) >= 1) {
			IntBPlusEntryInsertElement(cur, old_src_entry, insert_element, insert_element_child_id);
			break;
		}
		else {
		...
		if (!parent_pos) {
			...
			up_element_id = IntBPlusEntrySplit(tree, cur, cur_pos->entry_id, parent, -1, &src_entry, insert_element, cur_pos->element_iterator.cur_id, insert_element_child_id, &right_id, &up_element_iterator);
			IntBPlusElement* up_element = TEST_BPLUS_ELEMENT_REFERENCER_Reference(src_entry, up_element_id);
			IntBPlusEntryInsertElement(parent, src_entry, up_element, cur_pos->entry_id);
			;
			tree->root_id = parent_id;
			;
			if (src_entry->type == kBPlusEntryIndex) {
				IntBPlusEntryDeleteElement(src_entry, &up_element_iterator);
			}
			break;
		}
		IntBPlusEntry* parent = (parent_pos->entry_id);
		up_element_id = IntBPlusEntrySplit(tree, cur, cur_pos->entry_id, parent, parent_pos->element_iterator.cur_id, &src_entry, insert_element, cur_pos->element_iterator.cur_id, insert_element_child_id, &right_id, &up_element_iterator);
		;
		insert_up = 1;
	} while (0);
	;
	if (old_src_entry) {
		...
		if (old_src_entry->type == kBPlusEntryIndex) {
			IntBPlusEntryDeleteElement(old_src_entry, insert_element_iterator);
		}
		;
	}
	if (insert_up) {
		IntBPlusElement* up_element = TEST_BPLUS_ELEMENT_REFERENCER_Reference(src_entry, up_element_id);
		return IntBPlusTreeInsertElement(tree, iterator, src_entry, up_element, &up_element_iterator, cur_pos->entry_id);
	}
	return success;
}
static _Bool IntBPlusTreeDeleteElement(IntBPlusTree* tree, IntBPlusIterator* iterator) {
	...
	do {
		if ((((Int_BPlusEntryExtend*)((uintptr_t)entry - sizeof(Int_BPlusEntryExtend)))->count) >= (5 * 40 / 100)) {
			break;
		}
		if (!parent_pos) {
		...
		if ((((Int_BPlusEntryExtend*)((uintptr_t)sibling - sizeof(Int_BPlusEntryExtend)))->count) > (5 * 40 / 100)) {
			{
...
_Bool IntBPlusTreeInsert(IntBPlusTree* tree, IntBPlusLeafElement* element) {
	...
	_Bool success = IntBPlusTreeInsertElement(tree, &iterator, ((void*)0), (IntBPlusElement*)element, ((void*)0), (((void*)0)));
