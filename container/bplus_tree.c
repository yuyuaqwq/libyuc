...
* 向节点插入元素
*/
bool BPlusTreeInsertEntry(BPlusTree* tree, BPlusCursor* cursor, BPlusElement* insert_element) {
    ...
    if (insert_up) {
        return BPlusTreeInsertEntry(tree, cursor, &up_element);
    }
    return success;
}
...
* 删除指定节点的元素
*/
bool BPlusTreeDeleteEntry(BPlusTree* tree, BPlusCursor* cursor) {
    BPlusElementPos* cur_pos = BPlusCursorCur(tree, cursor);
    BPlusElementPos* parent_pos = BPlusCursorUp(tree, cursor);
    ...
    if (delete_up) {
        return BPlusTreeDeleteEntry(tree, cursor);     // 尾递归
    }
    return success;
}
