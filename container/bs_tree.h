...
//    LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Offset,
//    LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Offset,
//    LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id,
//    stack_count)
...
static LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id RotateLeft(BsTree* tree, BsEntry* sub_root_parent, LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id sub_root_id, BsEntry* sub_root) {
    LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id new_sub_root_id = accessor##_GetRight(tree, sub_root);
...
static LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id RotateRight(BsTree* tree, BsEntry* sub_root_parent, LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id sub_root_id, BsEntry* sub_root) {
    LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id new_sub_root_id = accessor##_GetLeft(tree, sub_root);
    if (new_sub_root_id == LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId) {
        return sub_root_id;
    }
    BsEntry* new_sub_root = LIBYUC_CONTAINER_BS_TREE_REFERENCER_Reference(tree, new_sub_root_id);
...
static void BsEntryInit(BsTree* tree, BsEntry* entry) {
    accessor##_SetLeft(tree, entry, LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId);
    accessor##_SetRight(tree, entry, LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId);
}
/*
* 初始化树
*/
...
bool BsTreeInsert(BsTree* tree, BsTreeStackVector* stack, LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id entry_id) {
    ...
    while (cur_id != LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId) {
        ...
        LIBYUC_CONTAINER_BS_TREE_Type_Key* cur_key = accessor##_GetKey(tree, cur);
        LIBYUC_CONTAINER_BS_TREE_Type_Key* entry_key = accessor##_GetKey(tree, entry);
        if (comparer##_Less(tree, cur_key, entry_key)) {
            if (accessor##_GetRight(tree, cur) == LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId) {
            ...
            if (accessor##_GetLeft(tree, cur) == LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId) {
...
LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id BsTreeDelete(BsTree* tree, BsTreeStackVector* stack, LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id entry_id, bool* is_parent_left) {
    ...
    if (accessor##_GetLeft(tree, entry) != LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId && accessor##_GetRight(tree, entry) != LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId) {
        ...
        LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id min_entry_id = accessor##_GetRight(tree, entry);
        ...
        while (accessor##_GetLeft(tree, min_entry) != LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId) {
        ...
        *new_entry_id = min_entry_id;
        ...
        LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id old_right_id = accessor##_GetRight(tree, min_entry);
        /* 最小节点可能是待删除节点的右节点 */
        ...
        if (accessor##_GetRight(tree, entry) !=    LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId) {
            /* 只有右子节点 */
            BsTreeHitchEntry(tree, parent, entry_id, accessor##_GetRight(tree, entry));
        }
        else if (accessor##_GetLeft(tree, entry) != LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId) {
            /* 只有左子节点 */
            BsTreeHitchEntry(tree, parent, entry_id, accessor##_GetLeft(tree, entry));
        }
        else {
            /* 没有子节点，直接从父节点中摘除此节点 */
...
LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id BsTreeIteratorFirst(BsTree* tree, BsTreeStackVector* stack) {
    ...
    BsEntry* cur = LIBYUC_CONTAINER_BS_TREE_REFERENCER_Reference(tree, cur_id);
    while (accessor##_GetLeft(tree, cur) != LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId) {
        BsTreeStackVectorPushTail(stack, &cur_id);
        cur_id = accessor##_GetLeft(tree, cur);
        LIBYUC_CONTAINER_BS_TREE_REFERENCER_Dereference(tree, cur);
        cur = LIBYUC_CONTAINER_BS_TREE_REFERENCER_Reference(tree, cur_id);
...
LIBYUC_CONTAINER_BS_TREE_REFERENCER_Type_Id BsTreeIteratorLast(BsTree* tree, BsTreeStackVector* stack) {
    ...
    BsEntry* cur = LIBYUC_CONTAINER_BS_TREE_REFERENCER_Reference(tree, cur_id);
    while (accessor##_GetRight(tree, cur) != LIBYUC_CONTAINER_BS_TREE_REFERENCER_Const_InvalidId) {
        BsTreeStackVectorPushTail(stack, &cur_id);
        cur_id = accessor##_GetRight(tree, cur);
        LIBYUC_CONTAINER_BS_TREE_REFERENCER_Dereference(tree, cur);
        cur = LIBYUC_CONTAINER_BS_TREE_REFERENCER_Reference(tree, cur_id);
