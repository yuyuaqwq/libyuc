...
void VectorResetCapacity(Vector* vector, LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Offset capacity) {
    LIBYUC_CONTAINER_VECTOR_Type_Element* new_buf = LIBYUC_CONTAINER_VECTOR_ALLOCATOR_CreateMultiple(arr, LIBYUC_CONTAINER_VECTOR_Type_Element, capacity);
    if (vector->obj_arr) {
        MemoryCopy(new_buf, vector->obj_arr, sizeof(LIBYUC_CONTAINER_VECTOR_Type_Element) * vector->count);
        LIBYUC_CONTAINER_VECTOR_ALLOCATOR_Release(vector, vector->obj_arr);
    }
    vector->obj_arr = new_buf;
    vector->capacity = capacity;
}
void VectorExpand(Vector* vector, LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Offset add_count) {
    LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Offset old_capacity = VectorGetCapacity(vector);
    LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Offset cur_capacity = old_capacity;
    LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Offset target_count = cur_capacity + add_count;
    if (cur_capacity == 0) {
        cur_capacity = 1;
    }
...
LIBYUC_CONTAINER_VECTOR_Type_Element* VectorPushTail(Vector* vector, const LIBYUC_CONTAINER_VECTOR_Type_Element* obj) {
    ...
    return VectorIndex(vector, (LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Id)VectorGetCount(vector) - 1);
}
LIBYUC_CONTAINER_VECTOR_Type_Element* VectorPushMultipleTail(Vector* vector, const LIBYUC_CONTAINER_VECTOR_Type_Element* obj, LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Offset count) {
    if (VectorGetCapacity(vector) <= (LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Id)VectorGetCount(vector) + (LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Id)count) {
#ifdef LIBYUC_CONTAINER_VECTOR_MODE_DYNAMIC
        VectorExpand(vector, count);
#endif
    }
    MemoryCopy(VectorIndex(vector, VectorGetCount(vector)), obj, sizeof(LIBYUC_CONTAINER_VECTOR_Type_Element) * count);
    VectorSetCount(vector, VectorGetCount(vector) + count);
    return VectorIndex(vector, (LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Id)VectorGetCount(vector) - (LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Id)count);
}
LIBYUC_CONTAINER_VECTOR_Type_Element* VectorGetTail(Vector* vector) {
...
LIBYUC_CONTAINER_VECTOR_Type_Element* VectorPopTail(Vector* vector) {
    ...
    VectorSetCount(vector, VectorGetCount(vector) - 1);
    return VectorIndex(vector, (LIBYUC_CONTAINER_VECTOR_REFERENCER_Type_Id)VectorGetCount(vector));
}


//LIBYUC_CONTAINER_VECTOR_Type_Element* VectorIteratorGetIndex(Vector* vector, VectorIterator* iter) {
//    if (iter->cur_index == LIBYUC_CONTAINER_VECTOR_REFERENCER_Const_InvalidId || iter->cur_index >= VectorGetCount(vector)) return NULL;
...
#undef LIBYUC_CONTAINER_VECTOR_Type_Element
