...
#define LIBYUC_CONTAINER_VECTOR_MODE_STATIC_DECLARATION_1(vector_type_name, count_type, element_type, static_count) \
    typedef struct _##vector_type_name##Vector { \
        count_type count; \
        element_type obj_arr[static_count]; \
    } vector_type_name##Vector;
#define LIBYUC_CONTAINER_VECTOR_MODE_STATIC_DECLARATION_2(vector_type_name, count_type) \
    void vector_type_name##VectorInit(vector_type_name##Vector* vector); 
#define LIBYUC_CONTAINER_VECTOR_MODE_STATIC_DEFINE_1(vector_type_name, count_type, id_type, element_type, allocator, callbacker) \
    bool vector_type_name##VectorIsEmpty(vector_type_name##Vector* vector) { \
        return vector->count == 0; \
    } \
    count_type vector_type_name##VectorGetCapacity(vector_type_name##Vector* vector) { \
        return sizeof(vector->obj_arr) / sizeof(vector->obj_arr[0]); \
    } \
    count_type vector_type_name##VectorGetCount(vector_type_name##Vector* vector) { \
        return vector->count; \
    } \
    void vector_type_name##VectorSetCount(vector_type_name##Vector* vector, count_type count) { \
        vector->count = count; \
    } \
...
#define LIBYUC_CONTAINER_VECTOR_MODE_DYNAMIC_DECLARATION_1(vector_type_name, count_type, element_type, static_count) \
    typedef struct _##vector_type_name##Vector { \
        count_type count; \
        count_type capacity; \
        element_type* obj_arr; \
    } vector_type_name##Vector;
#define LIBYUC_CONTAINER_VECTOR_MODE_DYNAMIC_DECLARATION_2(vector_type_name, count_type) \
    void vector_type_name##VectorResetCapacity(vector_type_name##Vector* vector, count_type capacity); \
    void vector_type_name##VectorExpand(vector_type_name##Vector* vector, count_type add_count); \
    void vector_type_name##VectorInit(vector_type_name##Vector* vector, count_type capacity); \
    void vector_type_name##VectorRelease(vector_type_name##Vector* vector);
#define LIBYUC_CONTAINER_VECTOR_MODE_DYNAMIC_DEFINE_1(vector_type_name, count_type, id_type, element_type, allocator, callbacker) \
    bool vector_type_name##VectorIsEmpty(vector_type_name##Vector* vector) { \
        return vector->count == 0; \
    } \
    count_type vector_type_name##VectorGetCapacity(vector_type_name##Vector* vector) { \
    ...
    void vector_type_name##VectorExpand(vector_type_name##Vector* vector, count_type add_count) { \
        count_type old_capacity = vector_type_name##VectorGetCapacity(vector); \
        count_type cur_capacity = old_capacity; \
        count_type target_count = cur_capacity + add_count; \
        if (cur_capacity == 0) { \
            cur_capacity = 1; \
        } \
...
#define LIBYUC_CONTAINER_VECTOR_MODE_INLINE_DYNAMIC_DECLARATION_1(vector_type_name, count_type, element_type, static_count) \
    typedef struct _##vector_type_name##Vector { \
        count_type count; \
        count_type capacity; \
        element_type obj_arr[]; \
    } *vector_type_name##Vector;
#define LIBYUC_CONTAINER_VECTOR_MODE_INLINE_DYNAMIC_DECLARATION_2(vector_type_name, count_type) \
    void vector_type_name##VectorResetCapacity(vector_type_name##Vector* vector, count_type capacity); \
    void vector_type_name##VectorExpand(vector_type_name##Vector* vector, count_type add_count); \
    void vector_type_name##VectorInit(vector_type_name##Vector* vector, count_type capacity); \
    void vector_type_name##VectorRelease(vector_type_name##Vector* vector);
...
#define LIBYUC_CONTAINER_VECTOR_MODE_INLINE_DYNAMIC_DEFINE_1(vector_type_name, count_type, id_type, element_type, allocator, callbacker) \
    bool vector_type_name##VectorIsEmpty(vector_type_name##Vector* vector) { \
...
#define LIBYUC_CONTAINER_VECTOR_DECLARATION(vector_type_name, mode, id_type, count_type, element_type, static_count) \
    mode##_DECLARATION_1(vector_type_name, count_type, element_type, static_count) \
    typedef struct _##vector_type_name##VectorIterator { \
        id_type cur_index; \
    } vector_type_name##VectorIterator; \
    mode##_DECLARATION_2(vector_type_name, count_type) \
    \
    count_type vector_type_name##VectorGetCount(vector_type_name##Vector* vector); \
    void vector_type_name##VectorSetCount(vector_type_name##Vector* vector, count_type count); \
    count_type vector_type_name##VectorGetCapacity(vector_type_name##Vector* vector); \
    ...
    element_type* vector_type_name##VectorIndex(vector_type_name##Vector* vector, id_type index); \
    \
    element_type* vector_type_name##VectorPushMultipleTail(vector_type_name##Vector* vector, const element_type* obj, count_type count); \
...
*/
#define LIBYUC_CONTAINER_VECTOR_DEFINE(vector_type_name, mode, id_type, count_type, element_type, key_type, accessor, allocator, callbacker, comparer, referencer) \
    mode##_DEFINE_1(vector_type_name, count_type, id_type, element_type, allocator, callbacker) \
    element_type* vector_type_name##VectorPushTail(vector_type_name##Vector* vector, const element_type* obj) { \
    ...
    \
    LIBYUC_ALGORITHM_ARRAY_DEFINE(vector_type_name##Vector, count_type, id_type, element_type, key_type, accessor, comparer, referencer) \
    \
    element_type* vector_type_name##VectorPut(vector_type_name##Vector* vector, const element_type* obj) { \
