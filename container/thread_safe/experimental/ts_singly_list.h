...
bool TsSinglyListIsEmpty(TsSinglyListHead* head) {
  return AtomicPtrLoad(&head->first) == NULL;
}

void TsSinglyListPutFirst(TsSinglyListHead* head, id_type entry) {
  do {
    id_type first = (id_type)AtomicPtrLoad(&head->first);
    entry->next = first;
    /* 读取可能拿不到最新的first(读取后其他线程完成了插入的第二步的设置/删除了该节点) */
    /* head->first = entry; */
  } while (!AtomicPtrCompareExchange(&head->first, AtomicPtrLoad(&entry), AtomicPtrLoad(&entry->next)));    /* 通过Cas保证entry->next依旧有效 */
}
void TsSinglyListDeleteFirst(TsSinglyListHead* head) {
  id_type old_first;
  do {
    old_first = (id_type)AtomicPtrLoad(&head->first);    /* 两个线程可能读取到同一个节点/期间插入了新节点 */
    /* head->first = old_first->next; */
  } while (!AtomicPtrCompareExchange(&head->first, AtomicPtrLoad(&old_first->next), (void*)old_first));    /* 通过Cas保证只有一个线程能够摘除该节点/期间没有插入新节点 */
}

id_type TsSinglyListIteratorFirst(TsSinglyListHead* head) {
  return (id_type)AtomicPtrLoad(&head->first);
}
id_type TsSinglyListIteratorNext(TsSinglyListHead* head, id_type entry) {
  id_type next_id = (id_type)AtomicPtrLoad(&entry->next);
  return next_id;
}
